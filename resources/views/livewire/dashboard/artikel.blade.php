...
state(['perPage', 'search', 'category', 'categories', 'sortBy', 'sortDir']);
...
with(fn () => ['articles' => Article::with('category')->search($this->search)->when($this->category !== '', function($query) {
    $query->where('category_id', $this->category);
})->orderBy($this->sortBy, $this->sortDir)->paginate($this->perPage)]);

$delete = function (Article $article){
    Storage::delete($article->image);
    session()->flash('success', 'Berhasil menghapus artikel');
    $article->delete();
};
...
<section>
    ...
    <div class="flex items-center justify-between">
        <h2 class="mb-1 text-3xl font-semibold md:mb-2">Artikel</h2>
        <a href="{{ route('artikelBuat') }}" wire:navigate
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            ...
                            <tr wire:key="{{ $article->id }}" class="border-b dark:border-gray-700">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 dark:text-white">
                                    <a target="_blank" href="{{ url('artikel') . '/' . $article->slug }}">
                                        {{ $article->title }}
                                    </a>
                                </th>
                                <td class="px-4 py-3 text-emerald-500">
                                    {{ $article->category->name }}</td>
                                <td class="px-4 py-3">{{ $article->excerpt }}</td>
                                <td class="px-4 py-3">{{ $article->created_at }}</td>
                                <td class="flex items-center justify-end gap-2 px-4 py-3">
                                    <button x-on:click="
                                    if(confirm('Are you sure you want to delete this post?')){ $wire.delete({{ $article->id }})}
                                    ...
                                    <a href="{{ url('dashboard/artikel/edit') . '/' . $article->slug }}" wire:navigate
