...
with(fn () => ['ustazs' => Ustaz::search($this->search)->orderBy($this->sortBy, $this->sortDir)->paginate($this->perPage)]);

$delete = function (Ustaz $ustaz){
    Storage::delete($ustaz->image);
    session()->flash('success', 'Berhasil menghapus ustaz');
    $ustaz->delete();
};
...
<section>
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('ustazBuat') }}" wire:navigate
            ...
            <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ($ustazs as $ustaz)
                            <tr wire:key="{{ $ustaz->id }}" class="border-b dark:border-gray-700">
                                <td class="px-4 py-3">
                                    <img class="w-32" src="{{ asset('storage') .'/'.$ustaz->image}}" alt="">
                                </td>
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 dark:text-white">
                                    {{ $ustaz->name }}
                                </th>
                                <td class="px-4 py-3">
                                    {{ $ustaz->description }}</td>
                                <td class="px-4 py-3">{{ $ustaz->created_at }}</td>
                                <td class="flex items-center justify-end gap-2 px-4 py-3">
                                    <button x-on:click="
                                    if(confirm('Are you sure you want to delete this data?')){ $wire.delete({{ $ustaz->id }})}
                                    ...
                                    <a href="{{ url('dashboard/ustaz/edit') . '/' . $ustaz->id }}" wire:navigate
