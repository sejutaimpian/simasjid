...
state(['perPage', 'search', 'sortBy', 'sortDir']);
...
with(fn () => ['dkms' => Dkm::search($this->search)->orderBy($this->sortBy, $this->sortDir)->paginate($this->perPage)]);

$delete = function (Dkm $dkm){
    Storage::delete($dkm->image);
    session()->flash('success', 'Berhasil menghapus dkm');
    $dkm->delete();
};
...
<section>
    ...
    <div class="flex items-center justify-between">
        <h2 class="mb-1 text-3xl font-semibold md:mb-2">Dewan Kemakmuran Masjid</h2>
        <a href="{{ route('dkmBuat') }}" wire:navigate
            ...
            <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                            @foreach ($dkms as $dkm)
                            <tr wire:key="{{ $dkm->id }}" class="border-b dark:border-gray-700">
                                <td class="px-4 py-3">
                                    <img class="w-32" src="{{ asset('storage') .'/'.$dkm->image}}" alt="">
                                </td>
                                ...
                                <td class="px-4 py-3">
                                    {{ $dkm->position }}</td>
                                <td class="px-4 py-3">{{ $dkm->created_at }}</td>
                                <td class="flex items-center justify-end gap-2 px-4 py-3">
                                    <button x-on:click="
                                    if(confirm('Are you sure you want to delete this data?')){ $wire.delete({{ $dkm->id }})}
                                    ...
                                    <a href="{{ url('dashboard/dkm/edit') . '/' . $dkm->id }}" wire:navigate
